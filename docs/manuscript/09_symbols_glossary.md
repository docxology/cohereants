...
**Response time analysis**: Compared using `src/core.py::calculate_response_time_improvement()`; see `tests/test_core.py::TestResponseTimeImprovement`. See `output/figures/response_time_comparison.png` and cf. \eqref{eq:response_time_components}. IR-specific timing is an experimental target, not an established value.
