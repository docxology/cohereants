...
def calculate_wavelength_from_wavenumber(
    wavenumber: Union[float, int, np.ndarray, List[float]],
    ...
    if wavenumber.size == 0:
        warnings.warn("Empty wavenumber array provided - returning empty wavelength array", UserWarning, stacklevel=2)
        return np.array([])
...
def calculate_atmospheric_transmission(
    wavelengths: Union[np.ndarray, List[float], float], distance: Union[float, np.ndarray] = None
) -> Union[float, np.ndarray]:
    """
...
def calculate_response_time_improvement(
    traditional_time: Union[float, np.ndarray], insect_time: Union[float, np.ndarray]
) -> Union[float, np.ndarray]:
    """
...
def safe_division(
    numerator: Union[float, np.ndarray], denominator: Union[float, np.ndarray], default: float = np.inf
) -> Union[float, np.ndarray]:
    """
