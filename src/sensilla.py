...
from .core import validate_numeric_inputs
...
class SensillaData:
    ...
    def __init__(self, lengths: Union[List[float], np.ndarray], diameters: Union[List[float], np.ndarray]):
        """
...
def analyze_sensilla_dimensions(lengths: List[float], diameters: List[float]) -> Dict[str, Union[np.ndarray, float]]:
    """
...
def generate_sensilla_visualization(
    lengths: List[float], diameters: List[float], save_path: Optional[str] = None, figsize: Tuple[int, int] = (12, 5)
) -> plt.Figure:
    """
    ...
    if len(data.lengths) > 1:
        ...
        p = np.poly1d(z)
        ax1.plot(data.diameters, p(data.diameters), "--", color=colors[2], alpha=0.8, linewidth=2)

    # Plot 2: Optimal detection wavelengths histogram
    if len(data.optimal_wavelengths_quarter) > 0:
        ax2.hist(
            data.optimal_wavelengths_quarter,
            bins=min(20, len(data.lengths)),
            alpha=0.7,
            label="1/4 λ resonance",
            color=colors[0],
            edgecolor="black",
        )
        ax2.hist(
            data.optimal_wavelengths_half,
            bins=min(20, len(data.lengths)),
            alpha=0.7,
            label="1/2 λ resonance",
            color=colors[1],
            edgecolor="black",
        )
        styler.format_axes(
            ...
            ylabel="Frequency",
            title="Optimal Detection Wavelengths",
        )
...
def calculate_sensilla_resonance_frequency(length: float, diameter: float, material_density: float = 1.0) -> float:
    """
...
def calculate_wavelength_matching(
    sensilla_lengths: Union[float, np.ndarray],
    incident_wavelengths: Union[float, np.ndarray],
    resonance_type: str = "quarter",
