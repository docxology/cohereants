...
def molecular_absorption_cross_section(
    wavelengths_um: np.ndarray, molecule: str = "H2O", temperature_k: float = 298.0, pressure_pa: float = 101325.0
) -> np.ndarray:
    """
    ...
    absorption_params = {
        ...
        "CH4": {"line_centers": [3.3, 7.7], "line_strengths": [1e-20, 5e-21], "line_widths": [0.2, 0.3]},
    }
    ...
    for center, strength, width in zip(params["line_centers"], params["line_strengths"], params["line_widths"]):
...
def rayleigh_scattering_coefficient(
    wavelengths_um: np.ndarray, pressure_pa: float = 101325.0, temperature_k: float = 298.0
) -> np.ndarray:
    """
...
def channel_capacity_analysis(
    ...
    if environmental_conditions is None:
        environmental_conditions = {"humidity": 50.0, "temperature": 298.0, "pressure": 101325.0, "visibility": 23.0}
    ...
    transmission_result = atmospheric_transmission_comprehensive(wavelengths, path_length_m, **env_params_mapped)
...
def optimize_wavelength_for_range(
    ...
    capacity_analysis = channel_capacity_analysis(
        wavelengths, target_range_m, signal_power_dbm, environmental_conditions=environmental_conditions
    )
...
def environmental_sensitivity_analysis(
    wavelength_um: float, path_length_m: float, parameter_ranges: Dict[str, Tuple[float, float]], n_points: int = 20
    ...
    """
    base_conditions = {"humidity": 50.0, "temperature": 298.0, "pressure": 101325.0, "visibility": 23.0}
...
def channel_capacity_vs_env(
    ...
    for i in range(len(h)):
        for j in range(len(t)):
            conditions = {"humidity": h[i], "temperature": t[j], "pressure": 101325.0, "visibility": 23.0}
