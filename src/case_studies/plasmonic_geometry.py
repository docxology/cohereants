...
def coupled_dipoles_near_field(
    ...
    single_particle = mie_scattering_sphere(
        np.array([wavelength_um]), radius_nm, epsilon_particle, epsilon_medium
    )
...
def optimize_plasmonic_geometry(
    ...
    epsilon_particle = drude_model_permittivity(
        np.array([target_wavelength_um]),
        params['plasma_wl'],
        params['collision_time']
    )[0]
    ...
    optimization_result = minimize_scalar(
        objective, bounds=size_range_nm, method='bounded'
    )
...
def sweep_plasmonic_quality(
    ...
    params = material_params[material]
    epsilon_material = drude_model_permittivity(
        wavelengths, params['plasma_wl'], params['collision_time']
    )
    ...
    for i, radius in enumerate(radii):
        ...
        mie_result = mie_scattering_sphere(
            wavelengths, radius, epsilon_material, medium_epsilon_real
        )
