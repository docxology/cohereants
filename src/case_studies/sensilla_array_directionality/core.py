...
def design_log_periodic_array(min_len_um: float, max_len_um: float, tau: float, count: int) -> np.ndarray:
    """
...
def sensilla_element_pattern(
    theta_deg: np.ndarray, length_um: float, wavelength_um: float, element_type: str = "dipole"
) -> np.ndarray:
    """
    ...
    if element_type == "dipole":
        ...
        pattern = np.where(theta_rad <= np.pi / 2, (np.sin(theta_rad)) ** 2, 0.0)
...
def mutual_coupling_matrix(
    positions: Union[np.ndarray, List[List[float]]], wavelength_um: float, coupling_strength: float = 0.1
) -> np.ndarray:
    """
    ...
    for freq_idx, wavelength in enumerate(wavelengths):
        ...
        for t_idx, theta in enumerate(np.deg2rad(theta_deg)):
            for p_idx, phi in enumerate(np.deg2rad(phi_deg)):
                ...
                element_pattern = sensilla_element_pattern(np.rad2deg([theta]), wavelength / 4, wavelength)[0]
